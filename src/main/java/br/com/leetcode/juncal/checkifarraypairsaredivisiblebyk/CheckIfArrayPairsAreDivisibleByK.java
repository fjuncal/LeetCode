...
public class CheckIfArrayPairsAreDivisibleByK {
    ...
    public static boolean canArrange3(int[] arr, int k) {
        ...
        for (int num : arr) {
            int remainder = ((num % k) + k) % k; // Garantir que o resto seja positivo
            remainderCounts[remainder]++;
        }

        // Verificar os pares
        if (remainderCounts[0] % 2 != 0) {
            return false; // Resto 0 deve ser par para formar pares
        }

        for (int i = 1; i <= k / 2; i++) {
            if (remainderCounts[i] != remainderCounts[k - i]) {
                return false; // Restos complementares devem ter a mesma contagem
            }
        }
